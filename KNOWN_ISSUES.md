...
| T6 | **Arapça sekme adlarında `ر` çizilmiyor** | Web denetim karesinde sekme çubuğu "الرئيسية" yerine "ال ئسسة", "القرآن" yerine "الق آن" çiziyor. **Yalnız sekme çubuğunda**; aynı ekranda gövde metnindeki `ر` ("الفجر", "العصر") doğru. Nedeni bulunamadı — izole olarak yeniden üretilemedi. Elenenler: yazı tipi yığını (4 yığın), punto (10–22), kalınlık (400–800), `letterSpacing`, dar kutu + `nowrap`/`ellipsis` (46 piksele kadar), `scaleX`, `-webkit-line-clamp`, etiketi react-navigation'ın `Text`inden tasarım sistemininkine taşımak. Hepsi izole testte **doğru** çiziyor. Sekme çubuğu iOS'ta CoreText, Android'de Minikin ile diziliyor; ikisi de Chromium'dan ayrı bir dizgi yolu, dolayısıyla cihazda görülme olasılığı düşük ama **kanıtlanmadı**. Arapça yayına açılmadan önce gerçek cihazda bakılmalı. |
