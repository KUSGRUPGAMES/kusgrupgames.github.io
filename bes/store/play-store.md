# Google Play — mağaza metinleri (BEŞ v1.0.0)

Play Console → Ana mağaza girişi.

> **Kimlik** — konsolda birebir bu değerler girilir:
> uygulama adı **BEŞ** · yayıncı **KUS GRUP GAMES** ·
> paket kimliği `com.kusgrup.bes` · destek `kusgrupgames@gmail.com` ·
> site `https://kusgrupgames.github.io/bes`.
> Paket kimliği yayından sonra değiştirilemez (DECISIONS D21).


> **App Store'dan farkı:** Play'de **ayrı anahtar kelime alanı yoktur**.
> Aranan kelimeler başlıkta, kısa açıklamada ve uzun açıklamada geçtiği
> kadar sayılır. Bu yüzden App Store'un gizli anahtar kelime alanına koyduğumuz
> kelimeler burada **uzun açıklamanın içine, cümlenin akışına** yedirilir.
> Kelime yığmak ters teper: Google başlıkta anahtar kelime doldurmasını
> cezalandırıyor.

## Uygulama adı (30 karakter sınırı)

```
BEŞ – Ezan & Namaz Vakitleri
```
(28 karakter)

İki mağazada aynı ad kullanılır: marka tek bir biçimde akılda kalsın diye.

## Kısa açıklama (80 karakter sınırı)

```
Kur’an, kıble, Elmalılı meali, dua ve zikirmatik — çevrimdışı çalışır
```
(69 karakter)

Başlıkta "Ezan" ve "Namaz Vakitleri" birlikte var; kısa açıklama Kur'an,
kıble ve zikri ekler. Play iki alanı da taradığı için hepsi kazanılır.

## Tam açıklama (4000 karakter sınırı)

```
BEŞ; ezan vakitlerini, kıble yönünü ve Kur’an-ı Kerim’in tamamını tek bir sade uygulamada toplar. Namaz vakitleri telefonunda güneşin konumundan hesaplanır — bir sunucuya sorulmaz, internet gerektirmez. Uçakta, yurt dışında, hattın çekmediği yerde de doğru çalışır.

★ HESAP YOK, SUNUCU YOK
Adınızı, e-postanızı, telefon numaranızı sormuyoruz. Konumunuz, ayarlarınız, ibadet kayıtlarınız, notlarınız ve zikir sayılarınız telefonunuzdan hiç çıkmıyor — bize ulaşmıyor, çünkü onları alacak bir sunucumuz yok.

YEDEK SİZDE
Bütün kayıtlarınızı tek bir dosyaya aktarıp yeni telefonunuza taşıyabilirsiniz. Dosya sizde kalır; uygulama onu hiçbir yere göndermez. Uygulamayı silerseniz hepsi onunla birlikte gider.

★ EZAN VE NAMAZ VAKİTLERİ
Sıradaki vakte saniye saniye geri sayım. Beş vakit ve güneş doğuşu tek bakışta, ay boyu vakit takvimi tek ekranda. Her vakit için ayrı bildirim kurabilirsiniz.

Yedi hesaplama yöntemi: Diyanet, Müslüman Dünya Birliği, ISNA, Mısır Genel Araştırma, Ümmü'l-Kurâ (Mekke), Karaçi ve Tahran. İkindi için Hanefî ve Şâfiî ölçüsü ayrı. Her vakte ayrı dakika düzeltmesi girip mahallenizin takvimiyle birebir tutturabilirsiniz.

★ KUR’AN-I KERİM — 6.236 ÂYETİN TAMAMI
Arapça metin Tanzil Project'ten birebir alınmıştır, değiştirilmemiştir. Türkçe meal, Elmalılı Hamdi Yazır'ın kamu malı eseridir ve uygulamayla birlikte gelir; okumak için internet gerekmez. Yalnız Arapça, Arapça + meal ya da yalnız meal olarak okuyun. Sure, cüz ve sayfa listeleri; kaldığınız yerden devam; renkli yer imleri ve âyet notları.

★ ON SEKİZ OKUYUCUDAN KIRAAT
Abdülbasit Abdüssamed, Mişarî Râşid el-Afâsî, Mahmud Halil el-Hüsarî, Abdurrahman es-Sudeys, Abdullah Basfar, Ali el-Huzeyfî ve on iki okuyucu daha. Murattal ve mücevved okuyuş. İnternetle dinleyin ya da sureleri indirip çevrimdışı dinleyin.

★ ARAMA
Arapça metinde ve mealde aynı anda arama. Harekesiz yazabilirsiniz; "2:255" gibi sure:âyet numarası da çalışır. Sure adı, dua, esmâ ve konu araması tek bir yerden.

★ KIBLE PUSULASI
Kâbe yönü ve kuş uçuşu uzaklık. Pusulanın doğruluğu düştüğünde ya da manyetik girişim olduğunda uygulama size söyler — yanlış yöne çevirmez. Pusula yalnız o ekran açıkken çalışır, pili boşuna harcamaz.

★ ZİKİRMATİK
Sekiz hazır zikir (Sübhânallah, Elhamdülillah, Allahü ekber, Lâ ilâhe illallah, Estağfirullah, Salavât, Hasbünallah, Lâ havle velâ kuvvete), altı hedef, kendi zikrinizi ekleme, her dokunuşta titreşim. Günlük, haftalık ve aylık istatistik, 14 günlük grafik ve seri takibi.

★ İBADET TAKİBİ
Kaza namazı sayaçları ve toplu giriş, gün gün ibadet defteri, oruç takibi (Ramazan, kaza, nafile), hatim ve mukabele planı, kendi hatırlatıcılarınız.

★ RAMAZAN
İmsak ve iftar geri sayımı, ay boyu imsakiye, oruç kaydı.

★ AYRICA
Zekât hesaplama (altın veya gümüş nisabı seçmeli, metodoloji ekranda yazılı), Esmâü'l-Hüsnâ, 14 kategoride dua, adım adım namaz rehberi, hicrî takvim ve yaklaşan dinî günler, Hac ve Umre rehberi, 43 maddelik İslami bilgi kütüphanesi, paylaşılabilir âyet kartları.

★ DİL DESTEĞİ
Arayüzün tamamı Türkçe, İngilizce, Arapça, Almanca ve Fransızca. Arapça seçildiğinde yazı yönü sağdan sola döner. Kur’an meali (Elmalılı) ve bilgi yazıları Türkçedir.

★ TAMAMEN ÜCRETSİZ, REKLAMSIZ
Abonelik yok, uygulama içi satın alma yok, reklam yok. Hiçbir özellik kilitli değil. Reklam kimliğiniz okunmuyor; uygulamada izleyici, piksel ya da analitik kütüphanesi bulunmuyor.

★ DÜRÜSTLÜK NOTU
Vakit hesabı yönteme, konuma ve rakıma bağlıdır; resmî ilanla bir iki dakika farklılık gösterebilir. Hicrî tarih aritmetik hesaplanır ve rüyete dayalı ilandan bir gün sapabilir. Kıble yönü telefonunuzun pusulasının doğruluğuna bağlıdır. Bunları gizlemiyor, ilgili ekranda yazıyoruz. BEŞ bir yardımcı araçtır; dinî hüküm (fetva) vermez ve bir din görevlisinin yerini tutmaz.
```

## Kategori

Uygulamalar → Yaşam Tarzı

## İçerik derecelendirmesi

Herkes (3+). Kullanıcı üretimi içerik yok, uygulama içi satın alma yok,
reklam yok.

## Veri güvenliği formu

**"Veri toplanmıyor" ve "veri paylaşılmıyor"** — konum, ibadet kayıtları ve
notlar cihazdan çıkmaz; hesap, reklam SDK'sı ve analitik yoktur. Konsolda
verilecek yanıtların tamamı `app-privacy.md` içinde yazılıdır.
